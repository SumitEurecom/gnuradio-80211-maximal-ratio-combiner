...
#include "soft_frame_equalizer_impl_dc.h"
...
soft_frame_equalizer_dc::sptr
soft_frame_equalizer_dc::make(Equalizer_soft_dc algo, double freq, double bw, int scaling, int threshold, bool log, bool debug) {
	return gnuradio::get_initial_sptr
		(new soft_frame_equalizer_impl_dc(algo, freq, bw, scaling, threshold, log, debug));
}
...
soft_frame_equalizer_impl_dc::~soft_frame_equalizer_impl_dc() {
}


void
soft_frame_equalizer_impl_dc::set_algorithm(Equalizer_soft_dc algo) {
...
void
soft_frame_equalizer_impl_dc::set_bandwidth(double bw) {
	gr::thread::scoped_lock lock(d_mutex);
	d_bw = bw;
}

void
soft_frame_equalizer_impl_dc::set_frequency(double freq) {
	gr::thread::scoped_lock lock(d_mutex);
	d_freq = freq;
}

void
soft_frame_equalizer_impl_dc::forecast (int noutput_items, gr_vector_int &ninput_items_required) {
	ninput_items_required[0] = noutput_items;
	ninput_items_required[1] = noutput_items;
}

int
soft_frame_equalizer_impl_dc::general_work (int noutput_items,
	...
	float *out1 = (float *) output_items[1]; // sd o/p b1
...
d_equalizer->equalize_soft(current_symbol, d_current_symbol, symbols, symbols_oai, noise_vec, d_scaling, d_threshold, out + o * 48,out1 + o * 48, d_frame_mod, d_frame_symbols);
d_equalizer->equalize_soft(current_symbol_1, d_current_symbol, symbols_1, symbols_oai_1, noise_vec, d_scaling, d_threshold, pout + o * 48,pout1 + o * 48, d_frame_mod, d_frame_symbols);
/*check point-2*/
...
for(int i = 0; i < 48; i++)
{
std::cout << "llr," << i << "," << (float)(out1+o)[i] << "," << (float)(pout1+o)[i] << std::endl;  
...
//		if(s_decode_signal_field(out1 + o * 48) && s_decode_signal_field(pout1 + o * 48)) // now it uses soft bits for decoding SIGNAL field
		...
		if(s_decode_signal_field_dc(out1 + o * 48, pout1 + o * 48))
		{
 
 //>std::cout << "decoded the signal field symInd--" << d_current_symbol << std::endl;
	pmt::pmt_t dict = pmt::make_dict();
	pmt::pmt_t dict_1 = pmt::make_dict();
	dict = pmt::dict_add(dict, pmt::mp("frame_bytes"), pmt::from_uint64(d_frame_bytes));
	dict_1 = pmt::dict_add(dict_1, pmt::mp("frame_bytes"), pmt::from_uint64(d_frame_bytes));
	dict = pmt::dict_add(dict, pmt::mp("encoding"), pmt::from_uint64(d_frame_encoding));
	dict_1 = pmt::dict_add(dict_1, pmt::mp("encoding"), pmt::from_uint64(d_frame_encoding));
	dict = pmt::dict_add(dict, pmt::mp("snr"), pmt::from_double(d_equalizer->get_snr_soft()));
	dict_1 = pmt::dict_add(dict_1, pmt::mp("snr"), pmt::from_double(d_equalizer->get_snr_soft()));
	dict = pmt::dict_add(dict, pmt::mp("freq"), pmt::from_double(d_freq));
	dict_1 = pmt::dict_add(dict_1, pmt::mp("freq"), pmt::from_double(d_freq));
	dict = pmt::dict_add(dict, pmt::mp("freq_offset"), pmt::from_double(d_freq_offset_from_synclong));
	dict_1 = pmt::dict_add(dict_1, pmt::mp("freq_offset"), pmt::from_double(d_freq_offset_from_synclong_1));
			...
			add_item_tag(1, nitems_written(0) + o,
						...
						dict,
						pmt::string_to_symbol(alias()));
			add_item_tag(2, nitems_written(0) + o,
						pmt::string_to_symbol("wifi_start"),
						dict_1,
						pmt::string_to_symbol(alias()));
			add_item_tag(3, nitems_written(0) + o,
						pmt::string_to_symbol("wifi_start"),
						dict_1,
						pmt::string_to_symbol(alias()));
		}
...
bool
soft_frame_equalizer_impl_dc::s_decode_signal_field(float *rx_scaled_llr) {
/*
	for(int i = 0; i < 48; i++)
	{
...
bool
soft_frame_equalizer_impl_dc::s_decode_signal_field_dc(float *rx_scaled_llr_b1, float *rx_scaled_llr_b2) {


	s_deinterleave_dc(rx_scaled_llr_b1, rx_scaled_llr_b2);
	memset(s_decoded_bytes,0,sizeof(s_decoded_bytes)); // 24 or 3 ? 
	memset(s_decoded_bits,0,sizeof(s_decoded_bits)); // 24 
        s_decoder.oai_decode(s_deinterleaved,s_decoded_bytes,s_decoded_bits,24);
...
void
soft_frame_equalizer_impl_dc::s_deinterleave(float *rx_scaled_llr) {
...
void
soft_frame_equalizer_impl_dc::s_deinterleave_dc(float *rx_scaled_llr_b1, float *rx_scaled_llr_b2) {
//TODO here convert the float llr to char llr lying from -8 to 7
	for(int i = 0; i < 48; i++) {
                if((rx_scaled_llr_b1[i]+rx_scaled_llr_b2[i]) > 7) 
		rx_scaled_llr_b1[i] = 7;
		else if ((rx_scaled_llr_b1[i]+rx_scaled_llr_b2[i]) < -8) 
  		rx_scaled_llr_b1[i] = -8;
		else
		rx_scaled_llr_b1[i] = (char)rx_scaled_llr_b1[i]+(char)rx_scaled_llr_b2[i];
		s_deinterleaved[i] = rx_scaled_llr_b1[interleaver_pattern[i]]; // is this correct to typecast here
		//{std::cout << "oai,"<<(int)s_deinterleaved[i] << std::endl;}
	}
}

bool
soft_frame_equalizer_impl_dc::parse_signal(uint8_t *decoded_bits) {
...
const int
soft_frame_equalizer_impl_dc::interleaver_pattern[48] = {
	 0, 3, 6, 9,12,15,18,21,
