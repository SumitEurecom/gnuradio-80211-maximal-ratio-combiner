...
sbmrc::sptr
sbmrc::make(Equalizer_sbmrc algo, double freq, double bw, int scaling, int threshold, bool log, bool debug) {
...
sbmrc_impl::~sbmrc_impl() {
}


void
sbmrc_impl::set_algorithm(Equalizer_sbmrc algo) {
...
/*
	case COMB:
		...
		break; */
	case LS_sbmrc:
		dout << "LS_sbmrc" << std::endl;
		d_equalizer = new equalizer_sbmrc::ls_sbmrc(); // defined in base_soft.h
		break;
/*	case LMS:
		dout << "LMS" << std::endl;
...
void
sbmrc_impl::set_bandwidth(double bw) {
	gr::thread::scoped_lock lock(d_mutex);
	d_bw = bw;
}

void
sbmrc_impl::set_scaling(int scaling) {
	d_scaling = scaling;
}

void
sbmrc_impl::set_threshold(int threshold) {
	d_threshold = threshold;
}

void
sbmrc_impl::set_frequency(double freq) {
	gr::thread::scoped_lock lock(d_mutex);
	d_freq = freq;
}
...
int
sbmrc_impl::general_work (int noutput_items,
...
void
sbmrc_impl::deinterleave(uint8_t *rx_bits) {
	for(int i = 0; i < 48; i++) {
...
void
sbmrc_impl::s_deinterleave(float *rx_scaled_llr) {
...
const int
sbmrc_impl::interleaver_pattern[48] = {
	 0, 3, 6, 9,12,15,18,21,
