...
class sync_short_dc_impl : public sync_short_dc {

public:
sync_short_dc_impl(double threshold, unsigned int min_plateau, bool log, bool debug) :
		block("sync_short_dc",
...
int ninput = std::min(std::min(ninput_items[2] ,std::min(ninput_items[0], ninput_items[1])), std::min(ninput_items[4], ninput_items[3]));
	...
	case COPY: {
		...
		dout << "SHORT copied EE" << o << std::endl;
...
sync_short_dc::sptr
sync_short_dc::make(double threshold, unsigned int min_plateau, bool log, bool debug) {
	return gnuradio::get_initial_sptr(new sync_short_dc_impl(threshold, min_plateau, log, debug));
}
