...
void ls_soft_dc::equalize_soft_dc(gr_complex *in, gr_complex *in_1, int n, gr_complex *symbols, gr_complex *symbols_1, gr_complex *symbols_oai, gr_complex *symbols_oai_1, float *noise_vec, float *noise_vec_1, int scaling, int threshold, float *llr, boost::shared_ptr<gr::digital::constellation> mod_soft, int d_frame_symbols) {
	...
	if(n == 0) 
	{
		...
		std::memcpy(d_H_soft_1, in_1, 64 * sizeof(gr_complex)); // copy 64 samples from channel 2 
	} 
	else if(n == 1) 
	{ 
                double signal = 0;
...
//TODO loop unrolling 	
		...
		{ 
			...
			if(i >= start && i <= stop)
			{
			noise_interf   += (std::pow(std::abs(d_H_soft[i]   - in[i]),   2));// ch1
			noise_interf_1 += (std::pow(std::abs(d_H_soft_1[i] - in_1[i]), 2));// ch2 
			}
			else
			{
			noise_non_interf   += (std::pow(std::abs(d_H_soft[i]   - in[i]),   2));// ch1  
			noise_non_interf_1 += (std::pow(std::abs(d_H_soft_1[i] - in_1[i]), 2));// ch2  
			}
...
//TODO shall I detect interf on one channel or both or the avg ? 
...
//TODO loop unrolling 
		for(int i = 0; i < 64; i++) 
		{
			if( (i == 11) || (i == 25) || (i == 32) || (i == 39) || (i == 53) || (i < 6) || ( i > 58)) 				{ 
				...
				symbols[c]   = in[i]   / d_H_soft[i];   // ch 1 equalize them with chest d_H
				symbols_1[c] = in_1[i] / d_H_soft_1[i]; // ch 2 equalize them with chest d_H
